window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 面板用浏览器直连 GitHub 公开 API（无 token）展示公开仓库统计；改成你的用户名。
    var GITHUB_USERNAME = "ZIye1208";
...
.ghmcp-page__stats { margin-top: 16px; }
.ghmcp-repo {
    ...
    function GitHubHint() {
      ...
      React.useEffect(function () {
        ...
        fetch(url, { headers: { Accept: "application/vnd.github+json" } }).then(function (res) {
          ...
          if (mounted) setStats({ repos: [], error: "读取公开仓库失败：" + String(e && e.message || e) });
        });
      ...
      if (stats) {
        ...
        statsEl = React.createElement("div", { className: "ghmcp-page__stats" },
          ...
          repoRows.length > 0
            ...
            : React.createElement("div", { className: "ghmcp-error" }, "没有可显示的公开仓库（或读取失败）"),
          stats.error ? React.createElement("div", { className: "ghmcp-error" }, String(stats.error)) : null
        );
      }
      ...
      return React.createElement("div", { className: "ghmcp-page" },
        ...
        React.createElement("div", { className: "ghmcp-page__status" }, "通过宿主配置接入 GitHub MCP → 工具以 mcp__github__* 形式出现。下面展示你的仓库统计；点示例可复制到剪贴板。"),
        statsEl,
