...
async function fetchJson(url, token) {
  ...
  if (res.status === 401) throw Object.assign(new Error("401 Unauthorized：token 无效或已过期"), { status: 401 });
  if (res.status === 403) throw Object.assign(new Error("403 Forbidden：token 缺少必要 scope 或账号被限制"), { status: 403 });
